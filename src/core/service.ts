...
async function _commitTx(tx: Tx): Promise<string | null> {
    ...
    if (!db || !db.get(tk)) {
        ...
        const resp = await jsonRpcReq('dece_commitTx', [signRet])
...
class TxState {
    ...
    async getAnchor(roots: Array<string>): Promise<Array<Witness> | null> {
        const resp: any = await jsonRpcReq('dece_getAnchor', [roots])
...
async function _fetchOuts(db: PopDB,info:any) {
    ...
    //     const resp:any = await jsonRpcReq('dece_blockNumber',['latest'])
